...
# There have been four successful IPA builds in an hour, and no way to tell
# from the running app which one is on the phone. So the app says.
write_build_info() {
  mkdir -p Generated
  local sha subject dirty
...
// Generated by Scripts/local-build.sh. Do not edit, do not commit.
enum BuildInfo {
...
# Order matters: build_catalog opens with `rm -rf Generated`, so the stamp has
# to be written after it, not before.
build_catalog
write_build_info
