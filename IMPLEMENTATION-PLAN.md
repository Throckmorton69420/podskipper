...
| B153 | Tapping an episode doesn't open its page (Apple does, everywhere). | **fixed (pass 18)** — `EpisodeRoute`/`ShowRoute` registered at each tab root; every episode row's text and cover open `EpisodeDetailView`, play stays its own button; previews of unfollowed shows open `PreviewEpisodeDetailView`. The Library's show tiles now push on the path (a presented show page broke value links). `testEpisodePage` green |
...
| B162 | (his phone, 23 Sep) Find Ads Again paused as soon as the screen dimmed; the pause symbol can't be pressed to resume. | **reworked (pass 19), unverified on the phone** — likely cause: backgrounding asked for the `processing` window, which ran `processPending` *beside* his job. Now one job at a time; only jobs he starts ask to carry on (Apple's rule) and appear on the Lock Screen; the processing window finishes or resumes his job first and does nothing else heavy off the charger; a rate-limited model question in the background waits instead of being lost; answers written to disk on leaving the app; a job iOS stops resumes by itself when the app opens (also after a relaunch), and the status sheet has Resume. Diagnostics → Working in the background records what iOS did (accepted/refused and why, started, ended early, model waits, bundle ID vs declared name, Background App Refresh) |
| B163 | (his phone, 23 Sep) Find Ads Again after the pause started over and stuck at 0 % on step 3/5 (finding ads); no way to restart a frozen step. | **fixed in code (pass 19), photographed in the simulator** — Find Ads Again called `process()` directly and ran beside the paused job (shared progress bar, model, answer cache); every button now goes through `processNow` (one job at a time). Each job owns its answer cache. The ad-free comparison is waited for at most 2 min. A watchdog (no progress and no model answer for 2 min with the app open) shows "No progress for 2 min" and **Restart** on the row, the activity card, the status sheet and the ⋯ menus; Restart keeps the transcript and answers |
| B164 | (his phone, 23 Sep) Scrolling stuttered right after installing the update, then settled. | **reduced (pass 19), unmeasured on the phone** — opening the app now staggers the catch-up: feeds at 4 s, back catalogues after, re-labelling at ~40 s; Apple-catalog merges at most 4 shows per refresh, one at a time, after the feeds; transcripts decoded off the main thread for the ad finder and re-labelling, with 5 s between re-labelled episodes |
| B165 | Player ⋯ menu has no Find Ads Again; episode actions differ between the row, the episode page and the player. | **fixed (pass 19)** — `EpisodeMenuItems` is the one list for the row ⋯, touch-and-hold, the episode page ⋯ and the player ⋯ (Play, Find Ads / Again / Resume / Restart, What Was Skipped, Up Next, Star, Mark Played, Download, Transcript, Chapters, Publish, Go to Show, Share, Share from Here (player), Copy Link, Episode Details). From the player, Go to Show and Episode Details close it and open the page in the tab behind (each tab now has a navigation path). Photographed |
