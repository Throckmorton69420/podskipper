...
| B120 | Detection: rethink the architecture (semantic classes, context, boundaries, confidence, learning); regression on MSSP 633. | **audited, not built** — claude/DETECTION-AUDIT.md. Lab baseline: MSSP 633 8/10 regions fail, Stavvy #199 9/9. Causes R1–R10 (45 s windows, discarded word times, keywords overriding the model, cue anchoring, coarse labels, same-kind fusing, bookend walk, no insertion-point audio, self-reported confidence, no boundary learning). Proposal P1–P7 |
| B121 | Edits must be feedback; keep original vs corrected vs locked. | **audited** — no original stored; edits overwrite; re-processing deletes edited/confirmed cuts; only thumbs record anything; thumbs-down is NOT required before editing (never was). Proposal P3/P4 |
| B122 | What Was Skipped editor like Photos / Voice Memos: independent playhead, transcript sync, precision. | **audited** — audition always restarts at the cut's start; no independent playhead; no transcript tap-to-seek; ~0.25 s/pt, no zoom; moving a handle while previewing breaks the preview state. Proposal P5 |
| B123 | Video source resolver; Stavvy's World #199 video. | **investigated** — feed has no video; Apple's public page links an open HLS on Simplecast's CDN that lines up with the feed audio (5,682.9 s vs 5,682 s); YouTube's channel feed is returning 404 everywhere today, so Watch on YouTube finds nothing; channel /videos page works. Decision needed on the Apple-page-linked stream. Proposal P6 |
